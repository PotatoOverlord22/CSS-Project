...
| **Exceptions for contracts** | Every constructor, state transition, and critical method uses `IllegalArgumentException` (for invalid input) or `IllegalStateException` (for invariant violations) to catch errors at runtime — always active regardless of JVM `-ea` flag. Configuration cross-validates that no process requires more memory than the total system memory. |
